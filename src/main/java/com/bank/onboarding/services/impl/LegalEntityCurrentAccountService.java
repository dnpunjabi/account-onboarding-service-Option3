...
@Slf4j
@Service
public class LegalEntityCurrentAccountService   implements CustomerProductService {
    ...
    @Autowired
    public LegalEntityCurrentAccountService (SchufaCheckFeatureImpl schufaCheckFeature,
    ...
    public String process(Map<String, Object> requestContext) throws Exception {
        try {
            // Step 1: Skip SCHUFA Check for Legal Persons (example rule)
            log.info("Skipping SCHUFA Check for Legal Entity Flex Account");
            ...
            log.info("Successfully completed onboarding for Legal Entity Flex Account.");
            return null;
        } catch (Exception ex) {
